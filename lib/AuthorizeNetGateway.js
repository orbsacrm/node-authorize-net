...
function createJsonCallback (cb) {
  ...
  return function (res, body) {
    return toJson(body, {arrayNotation: true})
  ...
  return sendXmlifiedRequest(this)(obj)
    .spread(createJsonCallback(function (json) {
      ...
      if (json.getSettledBatchListResponse) {
        ...
        if (json.getSettledBatchListResponse.messages[0].resultCode === 'Error') {
          throw new GatewayError('Failed to get batches', json.getTransactionListResponse);
        }

        batchList = json.getSettledBatchListResponse.batchList;

        if (!batchList) {
          return [];
        }

        batchList = batchList[0] ? batchList[0].batch : batchList.batch;
        batchList = batchList.length > 1 ? batchList : [
          batchList
        ];
        ...
        return batchList.map(function (batch) {
          ...
          var output = {
            ...
            settlementDate: batch.settlementTimeUTC[0],
            chargeAmount: 0,
            chargeCount: 0,
            refundAmount: 0,
            refundCount: 0,
          ...
          if (batch.statistics && batch.statistics[0] &&
              batch.statistics[0].statistic &&
              batch.statistics[0].statistic.length) {
            var statistics = batch.statistics[0].statistic;
            statistics.forEach(function(stat) {
              var statistic = {
                accountType: stat.accountType[0],
                chargeAmount: +(stat.chargeAmount[0]),
                chargeCount: +(stat.chargeCount[0]),
                refundAmount: +(stat.refundAmount[0]),
                refundCount: +(stat.refundCount[0]),
                voidCount: +(stat.voidCount[0]),
                declineCount: +(stat.declineCount[0]),
                errorCount: +(stat.errorCount[0]),
              };
              output.statistics.push(statistic);
              output.chargeAmount += statistic.chargeAmount;
              output.chargeCount += statistic.chargeCount;
              output.refundAmount += statistic.refundAmount;
              output.refundCount += statistic.refundCount;
              output.voidCount += statistic.voidCount;
              output.declineCount += statistic.declineCount;
              output.errorCount += statistic.errorCount;
            });
          }
  ...
  return P.resolve()
    ...
    .spread(createJsonCallback(function (json) {
      ...
      if (json.getTransactionListResponse) {
        ...
        transactions = json.getTransactionListResponse.transactions[0].transaction;
        return transactions.map(flattenObject);
      } else {
        throw new Error('Can not parse answer from gateway');
      }
    }));
